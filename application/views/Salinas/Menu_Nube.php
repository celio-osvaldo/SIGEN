...
<div class="nav flex-column menu_nube">
	<div class="row">
		<div class="col-md-2">
			...
			<?php
			$listar=null;
			$directorio=opendir('Resources/Nube_Sigen/SALINAS/');

			while ($elemento=readdir($directorio)) {
				if ($elemento!='.'&&$elemento!='..') {
					if(is_dir("Resources/Nube_Sigen/SALINAS/".$elemento))
					{
						?>
						<!--href="<?php echo base_url() ?>Resources/Nube_Sigen/SALINAS/<?php echo $elemento ?>/"-->
						...
						<a style="font-size: 0.65rem" class="nav-link" href="<?php echo base_url() ?>Resources/Nube_Sigen/SALINAS/<?php echo $ruta."/".$elemento ?>" download="<?php echo $elemento; ?>"  ><?php echo $elemento ?></a>
						<?php
					}
				}
			}
				...
				<div class="table-responsive">
					<table id="table_nube" class="table table-striped table-hover display" style="font-size: 10pt;">
						...
						<tbody>
							...
							if($ruta!=""){
								$listar=null;
								$directorio=opendir('Resources/Nube_Sigen/SALINAS/'.$ruta);
								...
								while ($elemento=readdir($directorio)) {
									if ($elemento!='.'&&$elemento!='..') {
										...
										<tr>
											<?php
											if(is_dir("Resources/Nube_Sigen/SALINAS/".$ruta."/".$elemento))
											{
												?>
												...
												<td><?php echo date ("d/m/Y H:i:s", filectime("Resources/Nube_Sigen/SALINAS/".$ruta."/".$elemento)); ?></td>
												<td>-</td>
												...
												?>
												<td> <a style="font-size: 1rem" class="nav-link" href="#" role="button" onclick="Carga_Vista(this.id)" id="Resources/Nube_Sigen/SALINAS/<?php echo $ruta."/".$elemento ?>"  ><?php echo $elemento ?></a>

												</td>

												<td><?php echo date ("d/m/Y H:i:s", filectime("Resources/Nube_Sigen/SALINAS/".$ruta."/".$elemento)); ?></td>

												<?php $size=filesize("Resources/Nube_Sigen/SALINAS/".$ruta."/".$elemento);
												if($size<1024){
													$size=1;
													$size_uni="KB";
												}else{
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$('#crearcarpeta').click(function(){
			...
			$.ajax({
				type:"POST",
				url:"<?php echo base_url();?>Salinas/Crea_Carpeta",
    ...
    $('#Solicita_borrado_carpeta').click(function(){
      ...
      if(txt_justifica!=""){
        $.ajax({
          type:"POST",
          url:"<?php echo base_url();?>Salinas/Solicita_Borra_carpeta",
    ...
    $('#Solicita_borrado_archivo').click(function(){
      ...
      if(txt_justifica!=""){
        $.ajax({
          type:"POST",
          url:"<?php echo base_url();?>Salinas/Solicita_Borra_archivo",
    ...
    $('#Solicita_descarga_archivo').click(function(){
      ...
      if(pass_descarga!=""){
        $.ajax({
          type:"POST",
          url:"<?php echo base_url();?>Salinas/Solicita_descarga_archivo",
	...
	function descargar($ruta){
		ruta="Resources/Nube_Sigen/SALINAS/"+$ruta;
		base_url="<?php echo base_url() ?>";
...
$(document).ready(()=>{
		...
		$("#add_archivo").on('submit', function(e){
			...
			$.ajax({
				...
				type: 'POST',
				url: '<?php echo base_url(); ?>Salinas/Add_File',
